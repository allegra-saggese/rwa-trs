"""
04_codebook.py -- WBES: generate CODEBOOK_WBES.xlsx, the survey's only codebook, in the Dropbox folder next to README.txt.
...
from wbes_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
metas = {w: json.load(open(LOGS / f"clean_{w}_meta.json")) for w in WAVES}
...
FILE_UNIVERSE = {"WBES_pooled_establishment.dta": "registered private non-agricultural establishments with five or more employees (manufacturing and most services), sampled by industry x size x region from the tax register; one row per establishment-round. The weight (strict eligibility) expands each round to its own universe; the rounds are separate cross-sections, and the World Bank panel id links the firms re-interviewed in 2006-2019"}
...
def codebook(fname, unit):
    ...
    nn, years, nrows = chunked_counts(P["final"] / fname, CLEAN.get("wave", "wave"))
    rows = []
    for c0 in df.columns:
        ...
        if c in orig and orig[c] and orig[c] != (vl.get(c0) or ""): src = (src + "; " if src else "") + f"original label: {orig[c][:60]}"
        vlab = vv.get(c0, {})
...
NOTES = [f"WBES codebook -- generated by rwa-trs/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the wbes_ prefix (at most 32 characters) and labelled in plain English starting with WBES (at most 80 characters); the mapping to the World Bank's own questionnaire codes and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv) and is repeated here in the columns native_name / native_label. A variable asked differently in different rounds carries the years in its name and label.",
         'Key block: wbes_survey wbes_year wbes_wave wbes_establishment_id wbes_country_firm_id wbes_panel_id wbes_panel_rounds wbes_sampling_region wbes_region wbes_locality_size wbes_sector_sampling wbes_sector_screener wbes_size_sampling wbes_size_screener wbes_stratum wbes_weight wbes_weight_median wbes_weight_weak (native: survey year wave idstd id panelid panel a2 a3a a3 a4a a4b a6a a6b strata wstrict wmedian wweak).',
         f"Alignment rule: rounds are grouped into versions of a variable by label similarity (token Jaccard >= {align['threshold']}); the largest group keeps the name, the others are <name>_v2, _v3 ...; forced alignments: {', '.join(sorted(align.get('force_align', []))) or 'none'}; forced splits: {', '.join(f'{k} ({v})' for k, v in (align.get('force_split') or {}).items()) or 'none'}.",
         ...
         'universe (files sheet): registered private non-agricultural establishments with five or more employees; the weight (wbes_weight, strict eligibility) expands each round to its own universe.',
         f"Processing notes (what z_Documentation says and how it was applied; every decision and why) are kept in the project memory file WBES.md (Green Jobs - TRS folder), not in git or Dropbox."]
books = [codebook("WBES_pooled_establishment.dta", "establishment")]
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
