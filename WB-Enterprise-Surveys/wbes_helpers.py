"""
wbes_helpers.py -- dataset-local helpers for the World Bank Enterprise Surveys pipeline.

Every dataset folder in this repository carries its own copy of this pattern (paths, logging,
Stata I/O, name/label hygiene) so each folder is independently replicable. Do not
import across dataset folders. The naming engine at the bottom is the block shared with the
NISR folders and is kept identical to them, on purpose.
"""
...
DATASET = "WB-Enterprise-Surveys"
HERE = Path(__file__).resolve().parent
LOGS = HERE / "logs"

# --------------------------------------------------------------------------- paths
# The Dropbox project folder sits at a different path on each collaborator's Mac.
# WBES_DB_ROOT (environment) overrides everything; otherwise the login user decides.
# The root is the project's data/ folder; the survey lives in data/WB-Enterprise-Surveys/.
ROOTS = {
    "matteo": "/Users/matteo/Library/CloudStorage/Dropbox/1-Ongoing Projects/Rwanda - TRS/data",
    "allegrasaggese": "/Users/allegrasaggese/Library/CloudStorage/Dropbox/Rwanda - TRS/data",
}

def db_root() -> Path:
    r = os.environ.get("WBES_DB_ROOT")
    if not r:
        u = getpass.getuser()
        if u not in ROOTS:
            sys.exit(f"Unknown user {u!r}: set WBES_DB_ROOT to the Rwanda - TRS/data folder, "
...
def get_logger(step: str) -> logging.Logger:
    """One log file per master run (WBES_RUN_ID); a step run on its own gets its own file."""
    LOGS.mkdir(exist_ok=True)
    run_id = os.environ.get("WBES_RUN_ID") or time.strftime("%Y%m%d-%H%M%S") + "_" + step
...
def read_dta(path, **kw):
    """pyreadstat with a latin1 fallback (several shipped files carry non-UTF-8 labels).
...
TRANSLATE_EXTRA = {}
KEY_EXTRA = {"idstd": ("establishment_id", "World Bank standard firm identifier, unique across countries and rounds"),
             "id": ("country_firm_id", "Firm identifier of the round as issued by the survey firm"),
             "panelid": ("panel_id", "Firm identifier shared across rounds by the World Bank panel"),
             "panel": ("panel_rounds", "Rounds in which the firm was interviewed"),
             "strata": ("stratum", "Sampling stratum: industry by size by region"),
             "stratificationregioncode": ("stratum_region", "Stratification region code"),
             "wstrict": ("weight", "Sampling weight under strict eligibility (the World Bank's default)"),
             "wmedian": ("weight_median", "Sampling weight under the median eligibility assumption"),
             "wweak": ("weight_weak", "Sampling weight under the weak eligibility assumption"),
             "a1": ("country_code", "Country code (33 = Rwanda)"),
             "a2": ("sampling_region", "Sampling region"), "a3": ("locality_size", "Size of the locality"),
             "a3a": ("region", "Region of the establishment"),
             "a4a": ("sector_sampling", "Industry sampling sector"), "a4b": ("sector_screener", "Industry screener sector"),
             "a6a": ("size_sampling", "Sampling size class"), "a6b": ("size_screener", "Screener size class"),
             "a7": ("multi_establishment", "The establishment is part of a multi-establishment firm")}
