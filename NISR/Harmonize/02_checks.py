...
02_checks.py -- NISR/Harmonize: independent verification of the harmonised copies -> logs/checks_report.txt
  ...
  The harmonised files carry the datasets' clean names (<dataset>_<english words>); every rule below is written
  against NISR's native names and the concept keys h_* (h_lfstatus, h_hhkey ...) and load() / names_of()
  translate through variable_names.csv and the shared stems, so the checks read next to the questionnaires.
  ...
  C. Concepts recomputed independently from the native items (second route, written from the documented code
     ...
     one head per household; LFS 2024 relationship codes 10-14.
...
from harmonize_helpers import (out_dir, h_dir, h_path, db_root, read_dta, read_meta, get_logger, LOGS, DATASETS,
...
row("A", "copies found in the wrong dataset folder (must be 0)", sum(1 for tag in DATASETS for f in h_dir(tag).glob("H_*.dta") if not f.name.startswith(f"H_{tag}_")), 0)
tested = 0
for out, info in summ["files"].items():
    ...
    if not HP(out).exists(): row("A", f"{out}: copy exists in {HP(out).parent.parent.name}/4_Harmonized", 0, 1); continue
    ms, mo = read_meta(src), read_meta(HP(out)); tested += 1
...
# The harmonised files carry the datasets' clean names; every check below is written against NISR's native names
# and the concept keys harmonised -- load() and names_of() translate, so the rules stay readable next to the questionnaires.
def _maps(out):
    info = summ["files"][out]; tag = info["dataset"]
    clean_of, _ = name_maps(tag, pathlib.Path(info["source"]).name)
    clean_of = {**clean_of, **{h: h_name(tag, h) for h in H_STEM}}
    return clean_of, {v: k for k, v in clean_of.items()}
def names_of(out):
    """the columns of a harmonised file under their native / h_ names"""
    clean_of, native_of = _maps(out); return [native_of.get(c, c) for c in read_meta(HP(out)).column_names]
def load(out, cols):
    clean_of, native_of = _maps(out); m = read_meta(HP(out))
    want = [c for c in (clean_of.get(x, x) for x in cols) if c in m.column_names]
    return read_dta(HP(out), usecols=want)[0].rename(columns=native_of)
hh_keys = {}
...
if "H_LFS_person.dta" in summ["files"] and not summ["files"]["H_LFS_person.dta"].get("skipped"):
    d = load("H_LFS_person.dta", ["wave", "sex", "status1", "h_lfstatus", "h_employed", "a02", "a02_v2", "h_relation", "h_hhkey", "wt", "age", "h_lfs_def"])
...
if "H_Census_person.dta" in summ["files"] and not summ["files"]["H_Census_person.dta"].get("skipped"):
    d = load("H_Census_person.dta", ["wave", "sex", "rp2024", "p23", "h_lfstatus", "p02", "p02_v2", "h_relation", "h_hhkey", "collective", "p21_v2", "h_marital", "p26", "p29_v2", "p06", "age"])
...
if "H_EICV_person.dta" in summ["files"] and not summ["files"]["H_EICV_person.dta"].get("skipped"):
    names = names_of("H_EICV_person.dta")
    ...
    d = load("H_EICV_person.dta", ["wave", "sex", "h_lfstatus", "h_relation", "h_hhkey", "econstatus", "h_educ", "age", "wt", "h_employed"] + lf + work3 + edu)
    if lf:
...
report += ["", "## D. Weighted employment-to-population ratio, persons aged 16+ (informational: definitions differ, see h_lfs_def)", "",
           "| section | dataset / wave | <ds>_labour_definition | employed 16+ / population 16+ with a status (%) | employed persons (weighted; LFS annual weights already average the rounds) | result |", "|---|---|---|---:|---:|---|"]
