"""
00_names.py -- AHS: build variable_names.csv (native name -> clean name and label, per file) from the last run's logs.
...
"""
import json, re, sys
from ahs_helpers import (paths, get_logger, LOGS, HERE, DATASET_TAG, NameTable, KEY_STEMS, KEY_EXTRA, WAVE_YEAR, TRANSLATE_EXTRA,
...
# ---------------------------------------------------------------- AHS: 13 pooled files (person / household in 3_Final, 11 modules appended), per-wave unit and module files
files = align["files"]; pooled_maps = {fname: name_pooled(fname, info["decisions"], None) for fname, info in files.items() if info.get("decisions")}
module_map = align.get("module_map", {})
for w, m in metas.items():
    unit_maps = [(f"pooled:AHS_pooled_{u}.dta", pooled_maps[f"AHS_pooled_{u}.dta"]) for u in ("person", "household") if f"AHS_pooled_{u}.dta" in pooled_maps]
    for u in ("person", "household"):
        if u in m and f"AHS_pooled_{u}.dta" in pooled_maps: name_wave_multi(f"wave:{w}:{u}", w, m[u]["vars"], m[u]["var_labels"], [(f"pooled:AHS_pooled_{u}.dta", pooled_maps[f"AHS_pooled_{u}.dta"])] + unit_maps)
    for stem, mm in m.get("modules", {}).items():
        canon = module_map.get(w, {}).get(stem); fn = f"AHS_pooled_{canon}.dta" if canon else None
        pooled = [(f"pooled:{fn}", pooled_maps[fn])] if fn in pooled_maps else []
