"""
04_codebook.py -- AHS: generate CODEBOOK_AHS.xlsx, the dataset's only codebook, in the Dropbox dataset folder next to README.txt.
...
import json, re, time
import pandas as pd
from ahs_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
FILE_UNIVERSE = {
    "AHS_pooled_person.dta": "all members of agricultural households (roster); education and main-activity items asked of members above 10 years in 2017; 2024 roster uses the EICV7 relationship codes",
    "AHS_pooled_household.dta": "agricultural households (2017: >= 1 member in crop or livestock production in 2016/17, listed in 1,560 segments; 2020: inverse-probability weighted sample; 2024: EICV7 households flagged agricultural in 600 EICV7 EAs)",
    "AHS_pooled_credits.dta": "households / members who requested a loan (2020 asked of members aged 16+)",
}
FILE_UNIVERSE_DEFAULT = 'agricultural households (2017: any member in crop/livestock production at listing; 2024: EICV7 households flagged agricultural) and their members / plots / animals'
...
NOTES = [f"AHS codebook -- generated by rwa-trs/NISR/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the ahs_ prefix (at most 32 characters) and labelled in plain English starting with AHS (at most 80 characters); the mapping to NISR's native names and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv) and is repeated here in the columns native_name / native_label. A variable that exists in several versions (different question or coding across waves) carries the years in its name and label; a sub-item whose NISR label is identical to another's keeps its item index or code as suffix.",
         'Key block (identical names in every NISR dataset): ahs_survey ahs_year ahs_wave ahs_sample ahs_province ahs_district ahs_urban ahs_cluster ahs_household_id ahs_person_id ahs_sex ahs_age ahs_weight ahs_household_weight (native: survey year wave sample prov dist urban cluster hhid pid sex age wt wt_hh).',
         ...
         'universe (files sheet): from the questionnaires and reports; frames differ by wave (2017 listing in 1,560 segments; 2020 inverse-probability weighted sample; 2024 EICV7 households flagged agricultural).',
...
# per-wave-only cleaned modules (no counterpart in another wave -> not appended; AHS audit): listed in the files sheet so they are discoverable
pooled_stems = {(w, s) for w, mp in align.get("module_map", {}).items() for s in mp}
for w, m in sorted(metas.items()):
    for s, i in sorted(m.get("modules", {}).items()):
        if (w, s) in pooled_stems and any(f == f"AHS_pooled_{align['module_map'][w][s]}.dta" for f in align["files"]): continue
        fname = f"AHS_{w}_{s}_clean.dta"
        if not (P["inter"] / fname).exists(): continue
        books.append((None, {"file": fname, "folder": "2_Intermediate", "unit": i.get("level", ""), "rows": i.get("rows"), "variables": i.get("vars"), "waves": w,
                             "universe": f"{w} only (per-wave section file, not appended across waves: either a base file that feeds the pooled person / household files or a section without a counterpart in another wave); {i.get('n_lsf', 0)} large-scale-farmer supplement rows (sample = LSF); variable labels in the .dta itself"}, []))
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/NISR/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
             ...
             f"Variable names: lower-case English words with the ahs_ prefix; labels in plain English starting with AHS. Native NISR names and labels: variable_names.csv in the git folder and the native_name / native_label columns of the codebook.",
