...
def codebook(fname, info):
    ...
    md = [f"## `{info.get('dir', '3_Final')}/{fname}` — one row per {unit}", "", f"{nrows:,} rows × {df.shape[1]} variables. Waves: {', '.join(map(str, waves))}.", "",
          "*Universe (questionnaires and reports, DOCUMENTATION.md):* " + FILE_UNIVERSE.get(fname, "agricultural households (2017: any member in crop/livestock production at listing; 2024: EICV7 households flagged agricultural) and their members / plots / animals"), "",
