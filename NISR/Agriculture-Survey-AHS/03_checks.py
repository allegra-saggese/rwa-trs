...
import pandas as pd
from ahs_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, NameTable, DATASET_TAG

log = get_logger("03_checks"); P = paths()
NAMES = NameTable(HERE / "variable_names.csv", DATASET_TAG)
CLEAN_P, CLEAN_H = NAMES.clean("pooled:AHS_pooled_person.dta"), NAMES.clean("pooled:AHS_pooled_household.dta")     # native -> clean; the checks keep NISR's native names
def read_native_cols(path, scope, natives):
    cl = NAMES.clean(scope); df, _, _ = read_dta(path, usecols=[cl[c] for c in natives]); return df.rename(columns=NAMES.native(scope))
...
person = read_native_cols(P["final"] / "AHS_pooled_person.dta", "pooled:AHS_pooled_person.dta", ["wave", "wt", "sex", "dist", "hhid"])
hh = read_native_cols(P["final"] / "AHS_pooled_household.dta", "pooled:AHS_pooled_household.dta", ["wave", "wt", "hhsize", "hhid", "head_sex"])
...
for w in ours:
    p = read_native_cols(P["inter"] / f"AHS_{w}_person_clean.dta", f"wave:{w}:person", ["wt", "hhid"]); dr, dw = DUP.get(w, 0), DUPW.get(w, 0.0)
...
clear all
set more off
use {" ".join(CLEAN_P[c] for c in ("wave", "wt", "sex", "hhid"))} using "{P["final"] / "AHS_pooled_person.dta"}", clear
rename ({" ".join(CLEAN_P[c] for c in ("wave", "wt", "sex", "hhid"))}) (wave wt sex hhid)
gen byte male = sex == 1
...
use {" ".join(CLEAN_H[c] for c in ("wave", "wt", "hhsize", "hhid"))} using "{P["final"] / "AHS_pooled_household.dta"}", clear
...
for f in mods:
    d = read_native_cols(P["root"] / align["files"][f]["dir"] / f, f"pooled:{f}", ["wave", "sample", "wt", "hhid"])
    cs = d[d["sample"] != "LSF"]
