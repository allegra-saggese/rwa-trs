...
def read_native(w, f):
    ...
    stem = f.name[len(f"AHS_{w}_"):-len("_clean.dta")]
    df, vl, vv = read_dta(f)
    m = METAS.get(str(w), {}); entry = m.get(stem) if stem in ("person", "household") else m.get("modules", {}).get(stem)
    if entry and entry.get("clean_names"):                                  # exact map written by 01_clean
