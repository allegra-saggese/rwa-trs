...
class Checks:
    """Collects PASS/FAIL checks; raise at the end so one failure never goes unnoticed."""
    def __init__(self, log): self.log, self.fails = log, []
    def __call__(self, cond: bool, msg: str, hard: bool = True):
        if cond: self.log.info("PASS  %s", msg)
        else:
            self.log.error("FAIL  %s", msg); self.fails.append(msg)
            if hard: raise AssertionError(msg)
    def done(self):
