...
from lfs_helpers import paths, get_logger, read_dta, LOGS, HERE, NameTable, DATASET_TAG
...
pcols = [NAMES.native(POOLED_P).get(c, c) for c in read_meta(P["final"] / "LFS_pooled_person.dta").column_names]
a02cols = [c for c in pcols if c == "a02" or c.startswith("a02_v")]
person, _, _ = read_dta(P["final"] / "LFS_pooled_person.dta", usecols=[CLEAN_P[c] for c in ["year", "wt", "wap16", "sex", "dist", "hhid", "interview"] + a02cols])
person = person.rename(columns=NAMES.native(POOLED_P))
hh, _, _ = read_dta(P["final"] / "LFS_pooled_household.dta", usecols=[CLEAN_H[c] for c in ["year", "wt", "hhsize", "hhid", "interview", "head_sex"]])
...
clear all
set more off
use {" ".join(CLEAN_P[c] for c in ("year", "wt", "wap16", "sex"))} using "{P["final"] / "LFS_pooled_person.dta"}", clear
rename ({" ".join(CLEAN_P[c] for c in ("year", "wt", "wap16", "sex"))}) (year wt wap16 sex)
...
use {" ".join(CLEAN_H[c] for c in ("year", "hhid", "hhsize"))} using "{P["final"] / "LFS_pooled_household.dta"}", clear
rename ({" ".join(CLEAN_H[c] for c in ("year", "hhid", "hhsize"))}) (year hhid hhsize)
