...
from lfs_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
NOTES = [f"LFS codebook -- generated by rwa-trs/NISR/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the lfs_ prefix (at most 32 characters) and labelled in plain English starting with LFS (at most 80 characters); the mapping to NISR's native names and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv) and is repeated here in the columns native_name / native_label. A variable that exists in several versions (different question or coding across years) carries the years in its name and label.",
         'Key block (identical names in every NISR dataset): lfs_survey lfs_year lfs_wave lfs_round lfs_quarter lfs_interview lfs_province lfs_district lfs_urban lfs_cluster lfs_psu lfs_household_id lfs_person_id lfs_sex lfs_age lfs_weight lfs_round_weight (native: survey year wave round quarter interview prov dist urban cluster psu hhid pid sex age wt wt_round).',
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/NISR/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
             ...
             f"Variable names: lower-case English words with the lfs_ prefix; labels in plain English starting with LFS. Native NISR names and labels: variable_names.csv in the git folder and the native_name / native_label columns of the codebook.",
