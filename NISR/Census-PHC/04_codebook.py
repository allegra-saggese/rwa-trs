...
from census_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
def codebook(fname, unit):
    ...
    for c0 in df.columns:
        c = NAT.get(c0, c0)                                            # native (NISR) name: the alignment log, sources and universes are keyed on it
        ...
        if c in orig and orig[c] and orig[c] != nat_label.get(c0, ""): src = (src + "; " if src else "") + f"2002 original label (FR): {orig[c][:60]}"
        vlab = vv.get(c0, {})
...
NOTES = [f"Census codebook -- generated by rwa-trs/NISR/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the census_ prefix (at most 32 characters) and labelled in plain English starting with CENSUS (at most 80 characters); the mapping to NISR's native names and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv; the 2002 French value labels are translated through value_label_translations.csv) and is repeated here in the columns native_name / native_label. A variable that exists in several versions (different question or coding across years) carries the years in its name and label.",
         'Key block (identical names in every NISR dataset): census_survey census_year census_wave census_province census_district census_sector census_urban census_cluster census_household_id census_person_id census_sex census_age census_weight census_household_weight (native: survey year wave prov dist sector urban cluster hhid pid sex age wt wt_hh).',
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/NISR/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
             ...
             f"Variable names: lower-case English words with the census_ prefix; labels in plain English starting with CENSUS. Native NISR names and labels: variable_names.csv in the git folder and the native_name / native_label columns of the codebook.",
