"""
00_names.py -- Census: build variable_names.csv (native name -> clean name and label, per file) from the last run's logs.
...
"""
import json, re, sys
from census_helpers import (paths, get_logger, LOGS, HERE, DATASET_TAG, NameTable, KEY_STEMS, KEY_EXTRA, WAVE_YEAR, TRANSLATE_EXTRA,
...
# ---------------------------------------------------------------- Census: pooled person (versions), pooled household (subset + hhsize/head_*/wt), per-year person files
pooled_map = name_pooled("Census_pooled_person.dta", align["decisions"], None)
table.by_scope_pooled = {r["native"]: r for r in table.rows}
hh_scope = "pooled:Census_pooled_household.dta"
