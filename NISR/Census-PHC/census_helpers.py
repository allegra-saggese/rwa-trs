...
DATASET_TAG = "census"
WAVE_YEAR = lambda w: int(str(w)[:4])                     # wave id -> census year
def _load_translations():
    """value_label_translations.csv next to the code: whole French value-label texts of the 2002 census (ISIC Rev. 3
    branches, ISCO-88 occupations, fields of study, continents) -> English; used by clean_value_label / clean_label"""
    f = HERE / "value_label_translations.csv"; out = {}
    if f.exists():
        with open(f, newline="", encoding="utf-8") as fh:
            for r in _csv.DictReader(fh):
                if r.get("source_text") and r.get("english"): out[r["source_text"].strip()] = r["english"].strip()
    return out
TRANSLATE_EXTRA = _load_translations()
KEY_EXTRA = {"collective": ("collective_household", "Collective (non-private) household or institution"),
             "hhid_nisr": ("nisr_household_id", "NISR household identifier as shipped"), "pid_nisr": ("nisr_person_id", "NISR person identifier as shipped"),
             "probability": ("selection_probability", "Selection probability of the household in the public-use sample"),
             "novprov": ("province_name_shipped", "Province name as shipped by NISR"), "novdistr": ("district_name_shipped", "District name as shipped by NISR"),
             "novsect": ("sector_name_shipped", "Sector name as shipped by NISR"), "hhsize": ("household_size", "Household size (persons in the public-use roster)")}
