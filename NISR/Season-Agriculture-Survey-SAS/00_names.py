"""
00_names.py -- SAS: build variable_names.csv (native name -> clean name and label, per file) from the last run's logs.
...
"""
import json, re, sys
from sas_helpers import (paths, get_logger, LOGS, HERE, DATASET_TAG, NameTable, KEY_STEMS, KEY_EXTRA, WAVE_YEAR, TRANSLATE_EXTRA,
...
# ---------------------------------------------------------------- SAS: 22 pooled files (3 in 3_Final), per-wave module files keyed year + "S/stem"
...
by_module = {}                                                   # (year, "S/stem") -> [(pooled scope, pooled_map, wave id)]
for fname, info in files.items():
    for w, mk in info.get("wave_modules", {}).items():
        if fname in pooled_maps: by_module.setdefault((str(mk[0]), mk[1]), []).append((f"pooled:{fname}", pooled_maps[fname], w))
for y, m in metas.items():
    for key, entry in m.items():
        season, stem = key.split("/", 1); cols = entry.get("columns") or list(entry.get("var_labels", {}))
        maps = by_module.get((str(y), key), [])
        # the pooled map is indexed by the pooled wave id (e.g. 2017_A_ssf); the per-wave scope uses the same id when there is one
        wave = maps[0][2] if maps else f"{y}_{season}"
        name_wave_multi(f"wave:{y}:{key}", wave, cols, entry.get("var_labels", {}), [(sc, pm) for sc, pm, _ in maps])
