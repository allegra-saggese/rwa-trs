"""
04_codebook.py -- SAS: generate CODEBOOK.md (+ codebook_<file>.csv) for every file in 3_Final/.
...
def codebook(fname, info):
    ...
    md = [f"## `{info.get('dir', '3_Final')}/{fname}` — one row per {unit}", "", f"{nrows:,} rows × {df.shape[1]} variables. Waves: {', '.join(map(str, waves))}.", "",
          "*Universe (questionnaires, DOCUMENTATION.md):* " + FILE_UNIVERSE.get(fname, "plots of sampled segments (area frame, strata 10 hillside / 20 marshland / 30 rangeland / 40 mixed) and of large-scale farmers (list frame, stratum 90); one record per plot x crop x season"), "",
...
out = ["# SAS codebook", "", "Generated by `04_codebook.py` from `3_Final/`. Do not hand-edit; see `DECISIONS.md` for the reasoning.", "",
