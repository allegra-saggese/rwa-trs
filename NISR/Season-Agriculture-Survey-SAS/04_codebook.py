...
import json, re, time
import pandas as pd
from sas_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
NOTES = [f"SAS codebook -- generated by rwa-trs/NISR/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the sas_ prefix (at most 32 characters) and labelled in plain English starting with SAS (at most 80 characters); the mapping to NISR's native names and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv) and is repeated here in the columns native_name / native_label. A variable that exists in several versions carries the years in its name and label; a sub-item whose NISR label is identical to another's keeps its item index or code as suffix.",
         'Key block: sas_survey sas_year sas_season sas_wave sas_farm_type sas_province sas_district sas_stratum sas_segment sas_holder sas_plot sas_crop sas_weight (native survey year season wave farm_type prov dist stratum segment holder plot crop wt), plus the CORE quantities sas_plot_area_ha sas_crop_area_ha sas_harvested_area_ha sas_production_kg sas_yield_kg_ha.',
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/NISR/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
             ...
             f"Variable names: lower-case English words with the sas_ prefix; labels in plain English starting with SAS. Native NISR names and labels: variable_names.csv in the git folder and the native_name / native_label columns of the codebook.",
