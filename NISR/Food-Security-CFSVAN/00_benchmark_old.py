"""00_benchmark_old.py -- CFSVA: no processed outputs existed before 2026-09-04; records raw file row counts. Idempotent."""
import json, sys
from cfsva_helpers import paths, read_meta, LOGS
OUT = LOGS / "benchmark_old_stata.json"
if OUT.exists(): print(f"benchmark already recorded at {OUT}"); sys.exit(0)
P = paths(); res = {"source": "no prior outputs; raw NISR/WFP files as shipped", "files": []}
for f in sorted(list(P["raw"].rglob("*.dta")) + list(P["raw"].rglob("*.sav"))):
