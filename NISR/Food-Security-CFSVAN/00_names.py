"""
00_names.py -- CFSVA: build variable_names.csv (native name -> clean name and label, per file) from the last run's logs.
...
"""
import json, re, sys
from cfsva_helpers import (paths, get_logger, LOGS, HERE, DATASET_TAG, NameTable, KEY_STEMS, KEY_EXTRA, WAVE_YEAR, TRANSLATE_EXTRA,
...
# ---------------------------------------------------------------- CFSVA: 4 pooled files (household, woman, child, village), per-wave unit files
...
for y, m in metas.items():
    for tag, entry in m.get("files", {}).items():
        unit = entry.get("unit", tag); fn = f"CFSVA_pooled_{unit}.dta"
        maps = [(f"pooled:{fn}", pooled_maps[fn])] if fn in pooled_maps else []
        cols = entry.get("columns") or entry.get("vars") or list(entry.get("var_labels", {}))
        name_wave_multi(f"wave:{y}:{tag}", y, cols, entry.get("var_labels", {}), maps)
