...
def codebook(fname, info):
    ...
    md = [f"## `{info.get('dir', '3_Final')}/{fname}` — one row per {unit}", "", f"{nrows:,} rows × {df.shape[1]} variables. Waves: {', '.join(map(str, waves))}.", "",
          "*Universe (methodology annexes and questionnaires, DOCUMENTATION.md):* " + FILE_UNIVERSE.get(fname, "records of the CFSVA household sample (district strata, PPS villages, 10 households per village)"), "",
