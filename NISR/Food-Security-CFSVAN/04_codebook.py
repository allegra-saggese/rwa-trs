"""
04_codebook.py -- CFSVA: generate CODEBOOK_CFSVA.xlsx, the dataset's only codebook, in the Dropbox dataset folder next to README.txt.
...
import json, re, time
import pandas as pd
from cfsva_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
FILE_UNIVERSE = {
    "CFSVA_pooled_household.dta": "households living in the sampled village at interview time (10 per village; 25 villages per district in 2012/2015, 30 in 2018/2024); 2009 unweighted",
    "CFSVA_pooled_woman.dta": "women aged 15-49 in sampled households (one record per woman; anthropometry for the same ages); 2015 records do not link to the household file",
    "CFSVA_pooled_child.dta": "children under 5 in sampled households (anthropometry 6-59 months, IYCF 6-23/24 months); 2015 records do not link to the household file",
    "CFSVA_pooled_village.dta": "one key-informant group interview per sampled village (village leaders, local government, teachers, health workers, farmers)",
}
FILE_UNIVERSE_DEFAULT = 'records of the CFSVA household sample (district strata, PPS villages, 10 households per village)'
...
def write_workbook(path, notes, books):
    ...
    mods = [(cb, m) for cb, m, _ in books if m["folder"] != "3_Final" and cb is not None]
    names = {m["file"]: sheet_name(m["file"], used) for _, m in finals}
    names.update({m["file"]: "modules" for _, m in mods})
    names.update({m["file"]: "(per-wave file; variables in its own .dta labels)" for cb, m, _ in books if cb is None})
...
def codebook(fname, info):
    ...
    nn, waves, nrows = chunked_counts(path, CLEAN.get("wave", "wave"))                    # chunked: pooled files can be several GB
    decisions = info.get("decisions", {}); rows = []
    unit = info.get("unit", "")
    for c0 in df.columns:
        c = NAT.get(c0, c0)                                            # native (NISR) name: decisions and sources are keyed on it
        ...
        others = ", ".join(f"{CLEAN.get(n, n)} ({', '.join(g)})" for n, g in vers.items() if n != c) if len(vers) > 1 else ""
        src = []
        for w in (ws_here or info.get("waves", [])):                     # lineage: clean_<wave>_meta.json -> files[unit].source (CFSVA audit: the old lookup skipped the 'files' level)
            sm = metas.get(w, {}).get("files", {}).get(unit, {}).get("source", {})
            if c in sm or base in sm: src.append(f"{w}: {sm.get(c, sm.get(base))}")
        src = "; ".join(src)
        vlab = vv.get(c0, {}); vtxt = ", ".join(f"{k}={v}" for k, v in list(vlab.items())[:5]) + (" …" if len(vlab) > 5 else "")
        conf = info.get("value_label_conflicts", {}).get(c)
        rows.append({"variable": c0, "label": vl.get(c0) or "", "native_name": c, "native_label": nat_label.get(c0, ""), "type": types.get(c0, ""), "value_labels": vtxt, "n_value_labels": len(vlab),
                     **{f"n_{w}": int(nn.loc[w, c0]) if w in nn.index and c0 in nn.columns else 0 for w in waves},
...
NOTES = [f"CFSVA codebook -- generated by rwa-trs/NISR/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the cfsva_ prefix (at most 32 characters) and labelled in plain English starting with CFSVA (at most 80 characters); the mapping to NISR's native names and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv; the Kinyarwanda labels of the 2009 wave are translated through value_label_translations.csv) and is repeated here in the columns native_name / native_label. A variable that exists in several versions carries the years in its name and label; a sub-item whose NISR label is identical to another's keeps its item code as suffix.",
         'Key block: cfsva_survey cfsva_year cfsva_wave cfsva_unit cfsva_province cfsva_district cfsva_sector cfsva_urban cfsva_cluster cfsva_household_id cfsva_weight (native survey year wave unit prov dist sector urban cluster hhid wt).',
         f"Alignment rule: a same-named variable is one column across waves only when (a) its variable labels are similar (token Jaccard >= {align['threshold']}) AND (b) its value labels are compatible (no code whose text means something else, similarity >= {align.get('vl_threshold', 0.6)}; an unlabelled wave's values inside the labelled range); the largest group keeps the name, the others are <name>_v2, _v3 ... (split reasons in logs/merge_alignment.json); forced alignments: {', '.join(sorted(align.get('force_align', []))) or 'none'}; forced splits: {', '.join(f'{k} ({v})' for k, v in (align.get('force_split') or {}).items()) or 'none'}.",
         ...
         'universe (files sheet): from the methodology annexes and questionnaires; 2018: the final report states 9,709 households interviewed (the annex target was 9,000); the village file has 987 rows against 749 documented interviews (left as shipped).',
         'Keys: cluster = village id in 2006 / 2012, the province-district-zone composite in 2009, a RECONSTRUCTED village group in the 2015 household file (households sharing the same embedded village answers), blank in 2018 / 2021 / 2024 (no PSU in the public files). 2015 woman / child rows carry nutrition-form ids (key, parent_key, mhn_key), NOT a household id. 2012 urban = NISR\'s 2-category urban_new (the native 3-category field is urban_nisr).',
...
# per-wave files that are not part of a pooled unit file (2009 questionnaire-section tables, 2012 linked child-mother-household file): listed in the files sheet (CFSVA audit)
pooled_units = {info.get("unit") for info in align["files"].values()}
for w, m in sorted(metas.items()):
    for tag, i in sorted(m.get("files", {}).items()):
        if i.get("unit", tag) in pooled_units and tag == i.get("unit", tag): continue
        if not (P["inter"] / i["out"]).exists(): continue
        books.append((None, {"file": i["out"], "folder": "2_Intermediate", "unit": i.get("unit", tag), "rows": i["rows"], "variables": len(i["vars"]), "waves": w,
                             "universe": f"{w} only, not pooled: " + ("NISR's linked child-mother-household analysis file (4,601 children with mother and household fields; 23 fields not in the separate files)" if tag == "child_mother_household" else f"household questionnaire section table '{tag}' (one row per item / member / crop / food record; keys hhid + the section's own item id); raw file {i['file']}") + "; variable labels in the .dta itself"}, []))
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/NISR/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
             ...
             f"Variable names: lower-case English words with the cfsva_ prefix; labels in plain English starting with CFSVA. Native NISR names and labels: variable_names.csv in the git folder and the native_name / native_label columns of the codebook.",
