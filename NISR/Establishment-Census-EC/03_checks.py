...
import pandas as pd
from ec_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, NameTable, DATASET_TAG
...
CLEAN = NAMES.clean(POOLED)        # native -> clean name in the pooled file; the checks below keep NISR's native names
...
cols = [c for c in ["year", "wt", "prov", "dist", "sector", "urban", "estid", "s04", "q3", "total_workers", "male_worker", "female_worker", "total_emp2", "q21c1", "q20",
                    "geo_conflict_2011", "prov_derived"] if CLEAN.get(c) in read_meta(P["final"] / "EC_pooled_establishment.dta").column_names]
est, _, _ = read_dta(P["final"] / "EC_pooled_establishment.dta", usecols=[CLEAN[c] for c in cols])
est = est.rename(columns=NAMES.native(POOLED))
...
clear all
set more off
use {" ".join(CLEAN[c] for c in ("year", "wt", "dist", "urban", "estid", "total_workers"))} using "{P["final"] / "EC_pooled_establishment.dta"}", clear
rename ({" ".join(CLEAN[c] for c in ("year", "wt", "dist", "urban", "estid", "total_workers"))}) (year wt dist urban estid total_workers)
gen long one = 1
