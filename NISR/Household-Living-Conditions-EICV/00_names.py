"""
00_names.py -- EICV: build variable_names.csv (native name -> clean name and label, per file) from the last run's logs.
...
# ---------------------------------------------------------------- EICV: 32 pooled files (person / household CS and VUP, 28 modules), 2 link files, per-wave unit and module files
files = align["files"]; pooled_maps = {}
LINK_STEMS = {"EICV3_4_panel_link.dta": ("EICV3_4_Panel", "data_stata"), "EICV5_vup_panel_link.dta": ("EICV5_VUP", "panel_for_merge")}
for fname, info in files.items():
    if info.get("decisions"): pooled_maps[fname] = name_pooled(fname, info["decisions"], None)
    else:
        w, stem = LINK_STEMS[fname]; mm = metas[w]["modules"][stem]
        name_plain(f"pooled:{fname}", info["vars"], mm.get("var_labels", {}), w)
for w, m in metas.items():
    vup = m.get("sample") == "VUP"; unit_file = {u: f"EICV_pooled_{u}{'_vup' if vup else ''}.dta" for u in ("person", "household")}
    unit_maps = [(f"pooled:{unit_file[u]}", pooled_maps[unit_file[u]]) for u in ("person", "household") if unit_file[u] in pooled_maps]
    for u in ("person", "household"):
        if u in m: name_wave_multi(f"wave:{w}:{u}", w, m[u]["vars"], m[u]["var_labels"], [(f"pooled:{unit_file[u]}", pooled_maps[unit_file[u]])] + unit_maps if unit_file[u] in pooled_maps else unit_maps)
    for stem, mm in m.get("modules", {}).items():
        pooled = [(f"pooled:{fn}", pooled_maps[fn]) for fn, info in files.items() if fn in pooled_maps and stem in info.get("stems", {}).get(w, [])]
