...
import json, re, time
import pandas as pd
from eicv_helpers import paths, get_logger, read_dta, read_meta, LOGS, HERE, DATASET, NameTable, DATASET_TAG
...
def codebook(fname, info):
    ...
    for c0 in df.columns:
        c = NAT.get(c0, c0)                                            # native (NISR) name: decisions, sources and universes are keyed on it
        ...
        # universe: unit files carry it per variable; module files per module stem (recorded by 01_clean from the questionnaires)
        wv = ws_here if ws_here else info.get("waves", [])
        if unit in ("person", "household"): uni = {w: metas[w][unit]["universe"][base] for w in wv if w in metas and unit in metas[w] and base in metas[w][unit].get("universe", {})}
        else: uni = {w: metas[w]["modules"][stem]["universe"] for w in wv for stem in info.get("stems", {}).get(w, []) if w in metas and stem in metas[w]["modules"] and metas[w]["modules"][stem].get("universe")}
        utxt = ("; ".join(f"{w}: {t}" for w, t in uni.items()) if len(set(uni.values())) > 1 else next(iter(uni.values()), "")) if uni else ""
        vlab = vv.get(c0, {}); vtxt = ", ".join(f"{k}={v}" for k, v in list(vlab.items())[:5]) + (" …" if len(vlab) > 5 else "")
        conf = info.get("value_label_conflicts", {}).get(c)
        rows.append({"variable": c0, "label": vl.get(c0) or "", "native_name": c, "native_label": nat_label.get(c0, ""), "type": types.get(c0, ""), "value_labels": vtxt, "n_value_labels": len(vlab), "universe": utxt,
...
NOTES = [f"EICV codebook -- generated by rwa-trs/NISR/{DATASET}/04_codebook.py on {time.strftime('%Y-%m-%d %H:%M')} from 3_Final/ (and 2_Intermediate/appended/). Do not hand-edit: re-run python master.py 04.",
         ...
         "Names and labels: every variable is named in lower-case English words with the eicv_ prefix (at most 32 characters) and labelled in plain English starting with EICV (at most 80 characters); the mapping to NISR's native names and labels is variable_names.csv next to the code (reviewed hand entries in variable_name_overrides.csv) and is repeated here in the columns native_name / native_label. A variable that exists in several versions (different question or coding across rounds) carries the years in its name and label; a sub-item whose NISR label is identical to another's keeps its item code as suffix.",
         'Key block (identical names in every NISR dataset): eicv_survey eicv_year eicv_wave eicv_sample eicv_province eicv_district eicv_urban eicv_cluster eicv_household_id eicv_person_id eicv_sex eicv_age eicv_weight eicv_household_weight (native: survey year wave sample prov dist urban cluster hhid pid sex age wt wt_hh).',
...
def update_readme():
    ...
    lines = [start, f"Pipeline: rwa-trs/NISR/{DATASET}/  (python master.py)   last run: {time.strftime('%Y-%m-%d %H:%M')}",
             ...
             f"Variable names: lower-case English words with the eicv_ prefix; labels in plain English starting with EICV. Native NISR names and labels: variable_names.csv in the git folder and the native_name / native_label columns of the codebook.",
