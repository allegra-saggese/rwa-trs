...
- **Location** `$DB/Publicly-Available-NISR/<dataset>/4_Harmonized/` — `H_<dataset>_<unit>.dta`, one harmonised copy of every file in that dataset's `3_Final/` (16 in all, mirroring 3_Final exactly). The cross-dataset documents (README, codebook, `harmonization_map.csv`) are in `Publicly-Available-NISR/Harmonisation-docs/`, renamed 2026-09-07 from `Harmonized/` because it holds no data
